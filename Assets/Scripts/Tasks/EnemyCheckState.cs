...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Conditionals
{
    ...
    [Category("征服与抉择/敌军AI")]
    public class EnemyCheckState : ECSConditionalTask<EnemyCheckStateSystem, EnemyCheckStateComponent>, IReevaluateResponder
    {
        ...
        public ComponentType ReevaluateFlag => typeof(EnemyCheckStateReevaluateFlag);
        public System.Type ReevaluateSystemType => typeof(EnemyCheckStateReevaluateTaskSystem);
    ...
    public struct EnemyCheckStateReevaluateFlag : IComponentData, IEnableableComponent { }
    ...
    [DisableAutoCreation]
    public partial struct EnemyCheckStateReevaluateTaskSystem : ISystem
    {
        private void OnUpdate(ref SystemState state)
        {
            var em = World.DefaultGameObjectInjectionWorld.EntityManager;
            ...
            foreach (var (taskBuf, compBuf) in
                SystemAPI.Query<DynamicBuffer<TaskComponent>, DynamicBuffer<EnemyCheckStateComponent>>()
                    .WithAll<EnemyCheckStateReevaluateFlag, EvaluateFlag>())
            {
                var tasks = taskBuf;
                for (int i = 0; i < compBuf.Length; ++i)
                {
                    ...
                    var newStatus = match ? TaskStatus.Success : TaskStatus.Failure;
